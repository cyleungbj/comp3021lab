...
public class Heap<T extends Comparable<T>> {
    ...
    public T poll() {
    	...
    	if(this.container.size() == 0) {
    		...
    		T first_element = this.container.get(0);
    		int last_element_pos = this.container.size()-1;
    		this.container.set(0, this.container.get(last_element_pos));
    		this.container.remove(last_element_pos);
    ...
    private void heapifyUp() {
    	...
    	while(hasParent(pos-1)) {
    		
    		int parentIndex = getParentIndex(pos);

    		
    		if(this.container.get(pos).compareTo(this.container.get(parentIndex)) > 0 ){ // larger than its parent
    			break;
    		}
    		else {
